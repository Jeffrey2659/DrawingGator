...
double getAlgoDist(Vector2d& curLoc, LegData& data) {
  ...
  switch (data.algo) {
    ...
    case CLW_ROTATE:
      ...
      Vector2d centerToPoint = (curLoc - data.center);
      Vector2d unitCenterToPoint = centerToPoint/(centerToPoint.Magnitude());
      Vector2d closestCircPoint = (unitCenterToPoint*radius) + data.center;
...
Vector2d getPointFromLengths(double leftLen, double rightLen) {
  double a = rightLen;
  double b = leftLen;
...
Vector2d getBestLengthDeltas(StateHolder& stateHolder) {
  ...
  for (int i = -2; i <= 2; i++) {
    for (int j = -2; j <= 2; j++) {
      ...
      Vector2d testPoint = getPointFromLengths(curLengths.X + lengths.X, curLengths.Y + lengths.Y);
